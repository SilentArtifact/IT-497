...
reviews <- jsonlite::fromJSON("http://api.rottentomatoes.com/api/public/v1.0/lists/dvds/current_releases.json?page_limit=50&page=1&country=us&apikey=pv8e8c9wkgkuwpsbpk72b8tg")
@
...
From the data here, we can see that the average critic's review of G-rated movies as a whole is \Sexpr{aveCriticGScore}\%, and R-rated movies as a whole, \Sexpr{aveCriticRScore}\%. The average audience's review of G-rated movies as a whole is \Sexpr{aveAudienceGScore}\%, and R-rated movies as a whole, \Sexpr{aveAudienceRScore}\%. This data can be clearly seen below:

<<calcFinalData, echo=FALSE, tidy=TRUE>>=
...
<<showFinalData, echo=FALSE, tidy=TRUE>>=
finalScores
@
